     ...
     document.location="../Farmer-Login.php";
        ...
        if (isset($_POST['Submit'])) {
            ...
            if ($image) {
                ...
                $image = 'Farmer_Images/' . $image;

                $stmt = $con->prepare("UPDATE users SET name = ?, password = ?, phone = ?, email = ?, image = ?, description = ? WHERE id = ? ");
                $stmt->bind_param("ssssssi", $name, $password, $phone, $email, $image, $description, $F_ID);

                if ($stmt->execute()) {

                    move_uploaded_file($_FILES["file"]["tmp_name"], "./Farmer_Images/" . $_FILES["file"]["name"]);
          ...
          document.location='./Account.php';
              ...
              $stmt = $con->prepare("UPDATE users SET name = ?, password = ?, phone = ?, email = ?, description = ? WHERE id = ? ");
              $stmt->bind_param("sssssi", $name, $password, $phone, $email, $description, $F_ID);
              ...
              <div class="card-body">
                ...
                <input type="hidden" name="F_ID" value="<?php echo $F_ID ?>">
                  ...
                  <div class="row mb-3">
                    ...
                    >
                    <div class="col-sm-10">
                      <textarea name="description" class="form-control" value="<?php echo $description ?>" id=""><?php echo $description ?></textarea>
                    </div>
                  </div>
